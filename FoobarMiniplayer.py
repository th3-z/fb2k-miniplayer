...
try:
    ...
    win32api.MessageBox(0, "Could not connect to f2k COM server.", "FoobarMiniplayer - Error")
    sys.exit()
...
class Application(Frame):
    def __init__(self, master=None):
        ...
        self.np = self.get_np() #Now playing (np)
    ...
    def create_widgets(self):
        ...
        self.rand_btn = Button(self, image=self.rand_img, bd=0, highlightthickness=0,
                               command = playback.Random)
        self.rand_btn.place(x=40,y=15)
        
        #Add a previous track button
        self.prev_btn = Button(self, image=self.prev_img, bd=0, highlightthickness=0,
        ...
        if playback.IsPaused:
            ...
            img = self.pause_img
        self.playpause_btn = Button(self, image=img, bd=0, highlightthickness=0,
                               command = playback.Pause)
        self.playpause_btn.place(x=90,y=15)
        
        #Add a next track button
        self.next_btn = Button(self, image=self.next_img, bd=0, highlightthickness=0,
                               command = playback.Next)
        self.next_btn.place(x=115,y=15)
        
        #Add a volume slider
        self.vol_scl = Scale(self,orient=HORIZONTAL,showvalue=0, sliderrelief=FLAT, highlightthickness=0,bd=0,
                             width=23,length=46,sliderlength=8,
                             from_=1, to=100,
                             bg=col_fg,
                             troughcolor=col_bg,
        ...
        #Add a text label
        self.marquee_lbl = Label(self, textvariable=self.marquee_text,
                            bg=col_bg, fg=col_fg,
    ...
    def updater(self):
        ...
        if self.marquee_lbl.winfo_width()>142: #Short strings wont need to be marqueed
            np_shifted = self.np[self.marquee_shift:] +", "+ self.np[:self.marquee_shift]
            self.marquee_text.set(np_shifted)
        
        if self.timer > 24: # 1 interation per 250ms, 24/4 = 6 seconds
        
            self.master.wm_attributes("-topmost", 1) #Sometimes a window might steal topmost
            self.timer = 0
        ...
        if self.marquee_lbl.winfo_width()>142 and not playback.IsPaused: #Marquee stops while paused
    ...
    def get_art(self):
        ...
        if not found_art:
            try:
                file = File(self.path)
                art_bytes = file.tags["APIC:"].data #Access APIC frame and grab bytes
                img = Image.open(io.BytesIO(art_bytes)) #Create new PIL image from bytes
                found_art = True
        ...
        if found_art and img is not None:
            img = img.resize((40,40), Image.ANTIALIAS) #Resize image
        else:
    ...
    def focus_fb(self, event=None):
        ...
        foobar = [(hwnd, title) for hwnd, title in winlist if "foobar2000" in title.lower()]
        foobar = foobar[0]
